...
*                 Default value : false;
*	 - sslCertificatePath: Absolute Path to SSL Certification Authority (CA) certificate 
...
function OnvifDevice(params) {
	...
	if(('useHttps' in params) && typeof (params['useHttps']) === 'boolean'){
		this.useHttps = params['useHttps'];
	}

	if(this.useHttps === true ){
		if(('sslCertificatePath' in params) && typeof (params['sslCertificatePath']) === 'string'){
			this.sslCertificatePath = params['sslCertificatePath'];
		}else{
			throw new Error('The parameter sslCertificatePath was invalid.');
		}
	}
	...
	if (('xaddr' in params) && typeof (params['xaddr']) === 'string') {
		...
		this.xaddr = this.useHttps ? "https://" : "http://";
		this.xaddr += this.address + '/onvif/device_service';
	} else {
		throw new Error('The parameter was invalid.');
	}
	...
	this.services = {
		'device': new mOnvifServiceDevice({ 'xaddr': this.xaddr, 'user': this.user, 'pass': this.pass, 'sslCertificatePath' : this.sslCertificatePath}),
		'events': null,
		'imaging': null,
		'media': null,
		'ptz': null,
...
/* ------------------------------------------------------------------
* Method: isAvailable([callback])
* Check device availabilty status
* ---------------------------------------------------------------- */
OnvifDevice.prototype.isAvailable = function (callback) {
	let promise = new Promise((resolve, reject) => {
...
OnvifDevice.prototype._getXaddr = function (directXaddr) {
	...
	return protocol + this.address + path;
}
...
OnvifDevice.prototype._getUri = function (directUri) {
	...
	let protocol = this.useHttps ? "https://" : "http://"
	const base = mUrl.parse(protocol + this.address);
