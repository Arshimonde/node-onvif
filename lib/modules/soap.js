...
* ---------------------------------------------------------------- */
/*
	- ssl_certificate_path : Absolute Path to SSL Certification Authority (CA) certificate 
...
/* ------------------------------------------------------------------
* Method: setSslCertificatePath(path)
* ---------------------------------------------------------------- */
OnvifSoap.prototype.setSslCertificatePath = function(path){
...
OnvifSoap.prototype._request = function(oxaddr, soap) {
	let promise = new Promise((resolve, reject) => {
		...
		let client = mHttp;
		if(oxaddr.protocol === "https:"){
			if(this.ssl_certificate_path && this.ssl_certificate_path !== "" && path.isAbsolute(this.ssl_certificate_path)){
				client = mHttps;
				post_opts.ca = fs.readFileSync(this.ssl_certificate_path);
				post_opts.port = oxaddr.port || 443
			}else{
				reject(new Error('SSL Certificate Error: '+ "an absolute path to SSL Certificate is required when using HTTPS URL / Request"));
			}
		}
		// end of implement https request
		let req = client.request(post_opts, (res) => {
